...
{
    [CustomEditor(typeof(BuildableGridObject))]
    public class BuildableGridObjectEditor : Editor
    {
        ...
        private Vector2IntField objectSizeRelativeToCellSizeField;

        private const float MIN_CELL_SIZE = 0;

        private void OnEnable()
        {
            buildableGridObject = (BuildableGridObject)target;
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            if (visualTreeAsset == null)
            {
                ...
                string uxmlPath = System.IO.Path.Combine(scriptDirectory, "BuildableGridObjectEditor.uxml");
                visualTreeAsset = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(uxmlPath);
            }
            visualTreeAsset.CloneTree(root);

            MonoScript script = MonoScript.FromMonoBehaviour((BuildableGridObject)target);
            ...
            objectSizeRelativeToCellSizeField = root.Q<Vector2IntField>("Object_Size_Relative_To_Cell_Size");
            objectSizeRelativeToCellSizeField.SetEnabled(false);
        ...
        {
            buildableGridObject.AutoCalculateSize();
            SetComponentDirty();
        }

        private void callCalculateAndDisplayDebugInfo(ClickEvent evt)
        {
            buildableGridObject.GetObjectSizeRelativeToCellSizeEditor();
            SetComponentDirty();
        }

        private void SetComponentDirty()
        {
            Undo.RecordObject(buildableGridObject, "Modified MyComponent");
            EditorUtility.SetDirty(buildableGridObject);
