...
{
    [CustomEditor(typeof(GridSaveAndLoadManager))]
    public class GridSaveAndLoadManagerEditor : Editor
    {
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            if (visualTreeAsset == null)
            {
                ...
                string uxmlPath = System.IO.Path.Combine(scriptDirectory, "GridSaveAndLoadManagerEditor.uxml");
                visualTreeAsset = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(uxmlPath);
            }
            visualTreeAsset.CloneTree(root);

            MonoScript script = MonoScript.FromMonoBehaviour((GridSaveAndLoadManager)target);
