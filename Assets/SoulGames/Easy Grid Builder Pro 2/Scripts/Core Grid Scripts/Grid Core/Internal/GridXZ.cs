...
namespace SoulGames.EasyGridBuilderPro
{
    ...
    public class GridXZ : Grid
    {
        ...
        public delegate void OnGridCellValueChangedDelegate(int x, int z);
        ...
        private CellPositionXZ[,] gridArray; // Multidimensional array representing the grid
        ...
        public GridXZ(int width, int length, float cellSize, Vector3 originPosition, Transform gridObjectTransform, int verticalGridIndex, GridDataHandler gridDataHandler)
        {
            ...
            gridArray = new CellPositionXZ[width, length];

            // Set up the grid
            InitializeGrid();
        }

        /// Initializes the grid by creating and assigning CellObjectXZ instances to each cell.
        private void InitializeGrid()
        {
            ...
            for (int x = 0; x < gridWidth; x++)
            {
                ...
                {
                    gridArray[x, z] = new CellPositionXZ(x, z);
                }
            }
        }
        ...
        public void InitializeRuntimeCanvasGrid(EasyGridBuilderPro.CanvasGridSettings canvasGridSettings, int thisGridVerticalIndex, float verticalGridHeight, GridMode activeGridMode, GridXZ activeGrid, GridOrigin gridOriginType)
        {
            ...
            runtimeCanvasGrid.eulerAngles = new Vector3(90, 0, 0);
        ...
        public void UpdateRuntimeCanvasGridPosition(float verticalGridHeight, int thisGridVerticalIndex, GridOrigin gridOriginType)
        {
            ...
            runtimeCanvasGrid.localPosition = new Vector3(offsetX, verticalGridHeight * thisGridVerticalIndex, offsetZ);
        }

        public void UpdateRuntimeCanvasGrid(EasyGridBuilderPro.CanvasGridSettings canvasGridSettings, int thisGridVerticalIndex, GridMode activeGridMode, GridXZ activeGrid)
        {
        ...
        private bool CanDisplayRuntimeCanvasGrid(EasyGridBuilderPro.CanvasGridSettings canvasGridSettings, int thisGridVerticalIndex, GridMode activeGridMode, GridXZ activeGrid)
        {
        ...
        public void InitializeRuntimeObjectGrid(EasyGridBuilderPro.ObjectGridSettings objectGridSettings, int thisGridVerticalIndex, float verticalGridHeight, GridMode activeGridMode, GridXZ activeGrid, BuildableObjectSO activeBuildableObjectSO, bool isRuntimeObjectGridHeatMapModeActivated, GridOrigin gridOriginType)
        {
        ...
        public void UpdateRuntimeObjectGridPosition(float verticalGridHeight, int thisGridVerticalIndex, GridOrigin gridOriginType)
        {
            ...
            runtimeObjectGrid.localPosition = new Vector3(offsetX, verticalGridHeight * thisGridVerticalIndex, offsetZ);
        }
        ...
        public void UpdateGeneratedTexture()
        {
            for (int x = 0; x < gridWidth; x++)
            {
                for (int z = 0; z < gridLength; z++)
                {
                    ...
                    runtimeObjectGridGeneratedTexture.SetPixel(x, z, color);
                }
            }
            runtimeObjectGridGeneratedTexture.Apply();
        }

        public void UpdateRuntimeObjectGrid(EasyGridBuilderPro.ObjectGridSettings objectGridSettings, int thisGridVerticalIndex, GridMode activeGridMode, GridXZ activeGrid, BuildableObjectSO activeBuildableObjectSO, bool isRuntimeObjectGridHeatMapModeActivated)
        {
        ...
        private bool CanDisplayRuntimeObjectGrid(EasyGridBuilderPro.ObjectGridSettings objectGridSettings, int thisGridVerticalIndex, GridMode activeGridMode, GridXZ activeGrid)
        {
        ...
        {
            Vector4 mousePosition = MouseInteractionUtilities.GetMouseWorldPositionWithCustomSurface(objectGridSettings.customSurfaceLayerMask, GridManager.Instance.GetGridSystemLayerMask(), Vector3.down * 9999, out _, out _);
            ...
            float offsetZ = ((mousePosition.z - gridObjectTransform.position.z) / (runtimeObjectGrid.localScale.z * maskSize)) * scaleFactor;
        ...
        public void InitializeRuntimeTextGrid(EasyGridBuilderPro.RuntimeTextGridSettings runtimeTextGridSettings, int thisGridVerticalIndex, float verticalGridHeight, GridMode activeGridMode, GridXZ activeGrid)
        {
            ...
            runtimeTextGrid.localPosition = new Vector3(0, verticalGridHeight * thisGridVerticalIndex, 0);
            ...
            for (int x = 0; x < gridWidth; x++)
            {
                for (int z = 0; z < gridLength; z++)
                {
                    runtimeTextGridTextObjectArray[x, z] = InstantiateTextObjectAtPosition(runtimeTextGridSettings, x, z);
                }
            }
        ...
        private Transform InstantiateTextObjectAtPosition(EasyGridBuilderPro.RuntimeTextGridSettings runtimeTextGridSettings, int x, int z)
        {
            ...
            {
                runtimeTextGridTextMeshProArray[x, z] = textMeshPro;
                ...
                if (runtimeTextGridSettings.displayCellPositionText)
                {
                    ...
                    else textMeshPro.text = gridArray[x, z].ToString();
                }
            }
            
            textObject.position = new Vector3(cellSize, 0, cellSize) / 2 + runtimeTextGridSettings.textGridCustomOffset + GetCellWorldPosition(x, z);
            textObject.parent = runtimeTextGrid;
            return textObject;
        }

        public void UpdateRuntimeTextGrid(EasyGridBuilderPro.RuntimeTextGridSettings runtimeTextGridSettings, int thisGridVerticalIndex, GridMode activeGridMode, GridXZ activeGrid)
        {
        ...
        private bool CanDisplayRuntimeTextGrid(EasyGridBuilderPro.RuntimeTextGridSettings runtimeTextGridSettings, int thisGridVerticalIndex, GridMode activeGridMode, GridXZ activeGrid)
        {
        ...
        {
            OnGridCellValueChanged += (int x, int z) =>
            {
                runtimeTextGridTextMeshProArray[x, z].text = gridArray[x, z].ToString();
            };
        }
        ...
        public void InitializeRuntimeProceduralGrid(EasyGridBuilderPro.RuntimeProceduralGridSettings settings, int thisGridVerticalIndex, float verticalGridHeight, GridMode activeGridMode, GridXZ activeGrid)
        {
            ...
            runtimeProceduralGrid.localPosition = new Vector3(0, verticalGridHeight * thisGridVerticalIndex, 0);
            ...
            for (int x = 0; x < gridWidth; x++)
            {
                for (int z = 0; z < gridLength; z++)
                {
                    InstantiateNode(settings, x, z, randomOffset);
                }
            }
            if (!settings.preserveSeedPostGeneration) Random.InitState((int)System.DateTime.Now.Ticks);
        }

        public void InstantiateNode(EasyGridBuilderPro.RuntimeProceduralGridSettings settings, int x, int z, Vector2 randomOffset)
        {
            int randomNodeIndex = GetRandomNodeIndex(settings, x, z, randomOffset);
            if (randomNodeIndex < 0) return;

            Transform nodePrefab = GetNodePrefab(settings, x, z, randomNodeIndex);
            ...
            SetNodeTransform(spawnedNode, settings, x, z, randomNodeIndex);
        }

        private int GetRandomNodeIndex(EasyGridBuilderPro.RuntimeProceduralGridSettings settings, int x, int z, Vector2 randomOffset)
        {
            bool isBorder = IsWithinBorders(settings, x, z);
            ...
            {
                float perlinValue = GetPerlinValue(settings, x, z, randomOffset);
        ...
        private void SetNodeTransform(Transform node, EasyGridBuilderPro.RuntimeProceduralGridSettings settings, int x, int z, int nodeIndex)
        {
            EasyGridBuilderPro.RuntimeProceduralGridSettings.NodeObjects nodeObject = IsWithinBorders(settings, x, z) ? settings.borderNodeObjects[nodeIndex] : settings.nodeObjects[nodeIndex];
            Vector3 offset = GetNodeOffset(nodeObject);
            float nodeSize = (cellSize / 100f) * nodeObject.nodeSizePercentage;

            node.localPosition = GetCellWorldPosition(x, z) + offset;
        ...
        private bool IsWithinBorders(EasyGridBuilderPro.RuntimeProceduralGridSettings settings, int x, int z)
        {
            int borderTileAmount = settings.borderTilesAmount;
            return x < borderTileAmount || x >= gridWidth - borderTileAmount || z < borderTileAmount || z >= gridLength - borderTileAmount;
        }

        private float GetPerlinValue(EasyGridBuilderPro.RuntimeProceduralGridSettings settings, int x, int z, Vector2 randomOffset)
        {
            float ValueX = (x + randomOffset.x + settings.scrollNoiseHorizontally) / gridWidth * settings.noiseScale;
            float ValueZ = (z + randomOffset.y + settings.scrollNoiseVertically) / gridLength * settings.noiseScale;
            return Mathf.PerlinNoise(ValueX, ValueZ);
        }

        private Transform GetNodePrefab(EasyGridBuilderPro.RuntimeProceduralGridSettings settings, int x, int z, int nodeIndex)
        {
            List<EasyGridBuilderPro.RuntimeProceduralGridSettings.NodeObjects> nodeObjectsList = IsWithinBorders(settings, x, z) ? settings.borderNodeObjects : settings.nodeObjects;
        ...
        public void UpdateRuntimeProceduralGrid(EasyGridBuilderPro.RuntimeProceduralGridSettings settings, int thisGridVerticalIndex, GridMode activeGridMode, GridXZ activeGrid)
        {
        ...
        private bool CanDisplayRuntimeProceduralGrid(EasyGridBuilderPro.RuntimeProceduralGridSettings settings, int thisGridVerticalIndex, GridMode activeGridMode, GridXZ activeGrid)
        {
        ...
        public bool IsWithinGridBounds(Vector2Int cellPosition)
        {
            ...
            return x >= 0 && z >= 0 && x < gridWidth && z < gridLength;
        }

        /// <summary>
        /// Checks if the provided cell position is valid within the grid boundaries.
        /// </summary>
        /// <param name="cellPositionXZ">The cell position to validate.</param>
        /// <returns>
        /// True if the cell position is within the boundaries of the grid; otherwise, false.
        /// </returns>
        public bool IsWithinGridBounds(CellPositionXZ cellPositionXZ)
        {
            int x = cellPositionXZ.x;
            int z = cellPositionXZ.z;
            return x >= 0 && z >= 0 && x < gridWidth && z < gridLength;
        }
        ...
        public bool IsWithinGridBoundsWithOuterPadding(Vector2Int cellPosition, Vector2Int outerPadding)
        {
            int x = cellPosition.x;
            int z = cellPosition.y;
            return x >= 0 && z >= 0 && x < gridWidth - outerPadding.x && z < gridLength - outerPadding.y;
        }

        /// <summary>
        /// Checks if the provided cell position is within the grid's boundaries considering specified outer padding.
        /// </summary>
        /// <param name="cellPositionXZ">The cell position to check.</param>
        /// <param name="outerPadding">The padding to consider outside the grid's boundaries.</param>
        /// <returns>
        /// True if the cell position is within the grid boundaries accounting for the outer padding; otherwise, false.
        /// </returns>
        public bool IsWithinGridBoundsWithOuterPadding(CellPositionXZ cellPositionXZ, Vector2Int outerPadding)
        {
            int x = cellPositionXZ.x;
            int z = cellPositionXZ.z;
            return x >= 0 && z >= 0 && x < gridWidth - outerPadding.x && z < gridLength - outerPadding.y;
        ...
        /// If the input position is outside the grid, it is clamped to the nearest valid position within the grid boundaries.
        /// </returns>
        public CellPositionXZ ClampToGridBounds(CellPositionXZ cellPositionXZ)
        {
            return new CellPositionXZ
            (
                Mathf.Clamp(cellPositionXZ.x, 0, gridWidth - 1),
                Mathf.Clamp(cellPositionXZ.z, 0, gridLength - 1)
            );
        }
        ...
        /// <param name="z">The z-coordinate of the grid cell that changed.</param>
        private void TriggerGridCellValueChanged(int x, int z)
        {
            // Invoke the OnGridObjectChanged event and pass the grid cell coordinates
            OnGridCellValueChanged?.Invoke(x, z);
        }
        ...
        //Return cell world position, by taking manual cell position (x, z)
        ...
        //Return cell world position, by taking cell position (x, z)
        public Vector3 GetCellWorldPosition(CellPositionXZ cellPositionXZ) 
        {
            return new Vector3(cellPositionXZ.x * cellSize , 0, cellPositionXZ.z * cellSize ) + originPosition;
        }

        //Return cell position (x, z), by taking cell world position
        public CellPositionXZ GetCellPosition(Vector3 worldPosition)
        {
            return new CellPositionXZ(
        ...
        //Return cell object, by taking manual cell position (x, z)
        public GridCellData GetCellData(int x, int z)
        {
            if (IsWithinGridBounds(new CellPositionXZ(x, z)))
            {
                return gridDataHandler.GetCellData(verticalGridIndex, new Vector2Int(x, z));
            }
            return default(GridCellData);
        }

        //Return cell object, by taking cell position (x, z)
        public GridCellData GetCellData(CellPositionXZ cellPositionXZ)
        {
            if (IsWithinGridBounds(cellPositionXZ))
            {
                return gridDataHandler.GetCellData(verticalGridIndex, new Vector2Int(cellPositionXZ.x, cellPositionXZ.z));
            }
            return default(GridCellData);
        }
        ...
        {
            CellPositionXZ cellPositionXZ;
            cellPositionXZ.x = GetCellPosition(worldPosition).x;
            cellPositionXZ.z = GetCellPosition(worldPosition).z;

            return GetCellData(cellPositionXZ);
        }
        ...
        private void ResizeGridArray()
        {
            ...
            CellPositionXZ[,] newGridArray = new CellPositionXZ[gridWidth, gridLength];
            ...
            {
                for (int z = 0; z < Mathf.Min(gridArray.GetLength(1), gridLength); z++)
                {
                    newGridArray[x, z] = gridArray[x, z];
                }
            }
        ...
        //Set a specified cell object to a specific cell (occupy cell), by taking manual cell position (x, z), and CellObjectXZ
        public void SetCellData(int x, int z, GridCellData gridCellData)
        {
            if (IsWithinGridBounds(new CellPositionXZ(x, z)))
            {
                gridDataHandler.SetCellData(verticalGridIndex, new Vector2Int(x, z), gridCellData);
                TriggerGridCellValueChanged(x, z);
            }
        }

        //Set a specified cell object to a specific cell (occupy cell), by taking cell position (x, z), and CellObjectXZ
        public void SetCellData(CellPositionXZ cellPositionXZ, GridCellData gridCellData)
        {
            if (IsWithinGridBounds(cellPositionXZ))
            {
                gridDataHandler.SetCellData(verticalGridIndex, new Vector2Int(cellPositionXZ.x, cellPositionXZ.z), gridCellData);
                TriggerGridCellValueChanged(cellPositionXZ.x, cellPositionXZ.z);
            }
        }

        //Set a specified cell object to a specified cell (occupy cell), by taking world position, and CellObjectXZ (This function converts world position to cell position and pass the values to the function above)
        public void SetCellData(Vector3 worldPosition, GridCellData value)
        {
            ...
            SetCellData(cellPositionXZ, value);
        }
        ...
        public override void SetRuntimeObjectGridGeneratedTextureCellToDefault(Vector2Int cellPosition)
        {
            if (cellPosition.x >= 0 && cellPosition.x < gridWidth && cellPosition.y >= 0 && cellPosition.y < gridLength)
            {
                ...
                runtimeObjectGridGeneratedTexture.SetPixel(adjustedX, adjustedZ, new Color(255,255,255,255));
                runtimeObjectGridGeneratedTexture.Apply();
            }
        }
        ...
        public override void SetRuntimeObjectGridGeneratedTextureCellColor(Vector2Int cellPosition, Color color)
        {
            if (cellPosition.x >= 0 && cellPosition.x < gridWidth && cellPosition.y >= 0 && cellPosition.y < gridLength)
            {
                int adjustedZ = gridLength - 1 - cellPosition.y;     // Flip z
                int adjustedX = gridWidth - 1 - cellPosition.x;      // Flip x
                runtimeObjectGridGeneratedTexture.SetPixel(adjustedX, adjustedZ, color);
                runtimeObjectGridGeneratedTexture.Apply();
            }
        }
