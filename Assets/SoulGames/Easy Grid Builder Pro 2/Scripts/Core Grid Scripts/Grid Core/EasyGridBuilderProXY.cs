...
{
    [AddComponentMenu("Easy Grid Builder Pro/Easy Grid Builder Pro XY", 1)]
    public class EasyGridBuilderProXY : EasyGridBuilderPro
    {
        #if UNITY_EDITOR
        private bool isEditorVisualHandlerComponentAdded = false;
        private const string EDITOR_GRID_VISUAL_HANDLER_XY = nameof(EditorGridVisualHandlerXY);
        #endif

        private GridManager gridManager;
        private GridDataHandler gridDataHandler;
        
        private GridXY activeGrid;
        private List<GridXY> gridList;
        ...
        private void DelayedAddEditorComponent()
        {
            ...
            if (transform.Find(EDITOR_GRID_VISUAL_HANDLER_XY) == null)
            {
                GameObject editorHandler = new GameObject(EDITOR_GRID_VISUAL_HANDLER_XY);
                editorHandler.transform.SetParent(transform, false);
                editorHandler.AddComponent<EditorGridVisualHandlerXY>();
            }
        }
        ...
        private void DelayedRemoveEditorComponent()
        {
            ...
            Transform editorHandler = transform.Find(EDITOR_GRID_VISUAL_HANDLER_XY);
        ...
        protected override void UpdateSafetyChecks()
        {
            ...
            {   
                ...
                {
                    Debug.Log($"EasyGridBuilderPro XY: {this.name}: <color=orange><b>Warning: Runtime Text Grid or Runtime Procedural Grid Enabled with Exceeded Safety Threshold Cell Count!</b></color> " +
        ...
        protected override void RunVisualGridsSafetyChecks()
        {
            ...
            if (displayRuntimeProceduralGrid && runtimeProceduralGridSettings.delayBetweenSpawns <= 0)
            {
                ...
                Debug.Log($"EasyGridBuilderPro XY: {this.name}: <color=red><b>Warning: Runtime Procedural Grid Exceeded Safety Threshold! Runtime Procedural Grid Disabled!</b></color> " +
            ...
            else if (displayRuntimeTextGrid)
            {
                ...
                Debug.Log($"EasyGridBuilderPro XY: {this.name}: <color=red><b>Warning: Runtime Text Grid Exceeded Safety Threshold! Runtime Text Grid Disabled!</b></color> " +
        ...
        protected override void SetupVerticalGrids()
        {
            ...
            gridList = new List<GridXY>();

            for (int i = 0; i < verticalGridsCount; i++)
            {
                Vector3 gridOrigin = CalculateGridOrigin(i);
                gridOriginList.Add(gridOrigin);
                gridList.Add(new GridXY(gridWidth, gridLength, cellSize, gridOrigin, transform, i, gridDataHandler));
            }
        ...
        protected override void SetupGridCollider()
        {
            ...
            float offsetY = (cellSize * gridLength / 2) + gridOriginList[0].y;  
            
            this.gridCollider.center = new Vector3(offsetX - transform.position.x, offsetY - transform.position.y, gridOriginList[0].z - transform.position.z);
            this.gridCollider.isTrigger = true;
            this.gridCollider.size = new Vector3((cellSize * gridWidth) * colliderSizeMultiplier, (cellSize * gridLength) * colliderSizeMultiplier, 0);
        }
        ...
        protected override void UpdateGriCollider()
        {
            ...
            if (lockColliderAtBottomGrid)
            {
                ...
                offsetY = (cellSize * gridLength / 2) + gridOriginList[0].y;  
                gridCollider.center = new Vector3(offsetX - transform.position.x, offsetY - transform.position.y, gridOriginList[0].z - transform.position.z);
            }
            else
            {
                offsetX = (cellSize * gridWidth / 2) + gridOriginList[activeVerticalGridIndex].x;
                offsetY = (cellSize * gridLength / 2) + gridOriginList[activeVerticalGridIndex].y;  
                gridCollider.center = new Vector3(offsetX - transform.position.x, offsetY - transform.position.y, gridOriginList[activeVerticalGridIndex].z - transform.position.z);
            }
        ...
        protected override void UpdateGridWidthAndLength()
        {
            if (updateGridWidthAndLengthRuntime && previousGridWidth != gridWidth || previousGridLength != gridLength)
            {
                ...
                foreach (GridXY gridXY in gridList)
                {
                    gridXY.SetGridSize(gridWidth, gridLength);
                }
            }
        }
            ...
            for (int i = 0; i < verticalGridsCount; i++)
            {
                if (runtimeProceduralGridSettings.nodeObjects.Count != 0 && !gridList[i].TryGetRuntimeProceduralGrid(out _))
                {
                    ...
                    for (int x = 0; x < gridWidth; x++)
                    {
                        ...
                        {
                            gridList[i].InstantiateNode(runtimeProceduralGridSettings, x, y, randomOffset);
        ...
        protected override void UpdateAutoDetectAndSetVerticalGrid()
        {
            ...
            Vector3 surfaceWorldPosition = MouseInteractionUtilities.GetMouseWorldPositionWithCustomSurface(autoDetectionLayerMask, customSurfaceLayerMask | gridManager.GetGridSystemLayerMask(),
                Vector3.forward * 9999, out _, out Vector3 firstCollisionWorldPosition);
        ...
        public override void SetInputBuildableFreeObjectSplinePlacementComplete()
        {
            ...
            while (iteration <= 1f)
            {
                Vector3 position = buildableFreeObjectSpline.EvaluatePosition(iteration);  // Get the position at the current t value
                ...
                if ((activeBuildableObjectSO as BuildableFreeObjectSO).objectRotateToSplineDirection) 
                {
                    ...
                    {
                        // XY grid: Adjust up direction to Z-axis
                        Quaternion baseRotation = Quaternion.LookRotation(tangent, Vector3.forward);
                        // Extract the rotation angle in the XY plane (about Z-axis)
                        freeRotation = baseRotation.eulerAngles.x;
                    }
                }

                SetInputBuildableObjectPlacement(worldPosition: position, freeRotation: freeRotation, invokedByFinalizedBoxPlacement: true, 
                    forceFreeObjectRotation: (activeBuildableObjectSO as BuildableFreeObjectSO).objectRotateToSplineDirection ? true : false);
                iteration += step;  // Increment "interation" to move to the next point on the spline
            }
        ...
        {
            Vector3 worldPosition = MouseInteractionUtilities.GetMouseWorldPositionWithCustomSurface(activeBuildableObjectSO.customSurfaceLayerMask, gridManager.GetGridSystemLayerMask(), Vector3.forward * 99999,
                out bool _, out Vector3 firstCollisionWorldPosition);
            worldPosition.z = firstCollisionWorldPosition.z;
        ...
        private Vector3 AdjustWorldPositionBasedOnCollisionForFreeObject(BuildableObjectSO activeBuildableObjectSO, out Vector3 hitNormals)
        {
            return MouseInteractionUtilities.GetMouseWorldPositionForBuildableFreeObject(activeBuildableObjectSO.customSurfaceLayerMask | gridManager.GetGridSystemLayerMask(), 
                (BuildableFreeObjectSO)activeBuildableObjectSO, Vector3.forward * 99999, out hitNormals);
        }
        ...
        private bool CheckPlacementDistance(Vector3 worldPosition)
        {
            ...
            {
                Debug.Log($"EasyGridBuilderPro XY: {this.name}: <color=red><b>Cannot Build Here!</b></color> Not Within The Provided Range! Distance: {distance}");
            }
            return false;
        }
        ...
        {
            Vector2Int startCell = GetActiveGridCellPosition(boxPlacementStartPosition);
            Vector2Int endCell = GetActiveGridCellPosition(boxPlacementEndPosition);
        ...
        {
            Vector2Int buildableObjectOriginCellPosition = new Vector2Int(cellPosition.x, cellPosition.y);
            Vector3 worldPosition = GetActiveGridCellWorldPosition(cellPosition);
            
            ICommand command = new CommandPlaceBuildableGridObject(this, buildableObjectOriginCellPosition, worldPosition, buildableGridObjectSO, direction, buildableObjectSORandomPrefab, 
        ...
        {
            List<Vector2Int> cellPositionList = buildableGridObjectSO.GetObjectCellPositionsList(originCellPosition, direction, cellSize, buildableObjectSORandomPrefab);

            if (TryPlaceBuildableGridObjectSinglePlacement(cellPositionList, originCellPosition, worldPosition, buildableGridObjectSO, direction, ref buildableObjectSORandomPrefab,
            ...
            else
            {
                if (gridManager.GetIsEnableConsoleMessages() && gridManager.GetConsoleMessagesSettings().onBuildableObjectPlacement)
                {
                    ...
                    Debug.Log($"EasyGridBuilderPro XY: {this.name}: <color=red><b>Cannot Build Here!</b></color> {message}");
                }
            }
        }

        private bool TryPlaceBuildableGridObjectSinglePlacement(List<Vector2Int> cellPositionList, Vector2Int buildableObjectOriginCellPosition, Vector3 worldPosition, BuildableGridObjectSO buildableGridObjectSO, 
        ...
        private void InitializeBuildableGridObjectPlacement(Vector2Int buildableObjectOriginCellPosition, Vector3 worldPosition, BuildableGridObjectSO buildableGridObjectSO, FourDirectionalRotation direction, 
            ...
            out BuildableGridObject buildableGridObject, BuildableGridObject originalBuildableGridObject = null)
        {
            ...
            Vector3 placedObjectWorldPosition = gridList[activeVerticalGridIndex].GetCellWorldPosition(buildableObjectOriginCellPosition.x, buildableObjectOriginCellPosition.y) + new Vector3(rotationOffset.x, rotationOffset.y, 0) * cellSize;
            placedObjectWorldPosition = new Vector3(placedObjectWorldPosition.x, placedObjectWorldPosition.y, worldPosition.z);
            ...
            buildableGridObject.SetupBuildableGridObject(this, activeVerticalGridIndex, cellSize, buildableGridObjectSO, buildableObjectSORandomPrefab, placedObjectWorldPosition, 
                buildableObjectOriginCellPosition, cellPositionList, direction);
        ...
        private void UpdateGridObjectBoxPlacement()
        {
            ...
            boxPlacementEndPosition = MouseInteractionUtilities.GetMouseWorldPositionWithCustomSurface(activeBuildableObjectSO.customSurfaceLayerMask, layerMask, Vector3.forward * 9999,
            ...
            BuildableObjectSO.RandomPrefabs buildableObjectSORandomPrefab = null, BuildableEdgeObject originalBuildableEdgeObject = null)
        {
            ...
            {
                InvokeTryPlaceBuildableEdgeObjectSinglePlacement(buildableObjectOriginCellPosition, buildableEdgeObjectSO, fourDirectionalDirection, isBuildableEdgeObjectFlipped, ref buildableObjectSORandomPrefab,
                    closestCornerWorldPosition, cornerObjectOriginCellDirection, ignoreCustomConditions, ignoreReplacement, verticalGridIndex, byPassEventsAndMessages, out spawnnedBuildableEdgeObject, 
                    originalBuildableEdgeObject: originalBuildableEdgeObject);
            }
            ...
            out BuildableEdgeObject buildableEdgeObject, Vector3 offset = default, BuildableEdgeObject originalBuildableEdgeObject = null)
        {
            ...
            Dictionary<CellPositionXY, EdgeObjectCellDirection> cellPositionsDictionary = GetEdgeObjectCellPositionDictionary(new CellPositionXY(buildableObjectOriginCellPosition.x, buildableObjectOriginCellPosition.y), 
        ...
        public Dictionary<CellPositionXY, EdgeObjectCellDirection> GetEdgeObjectCellPositionDictionary(CellPositionXY buildableObjectOriginCellPosition, CornerObjectCellDirection originCornerObjectCellDirection,
        FourDirectionalRotation fourDirectionalDirection, int objectLengthRelativeToCellSize)
        {
            Dictionary<CellPositionXY, EdgeObjectCellDirection> cellPositionsDictionary = new Dictionary<CellPositionXY, EdgeObjectCellDirection>();
        ...
        private void HandleEdgeObjectNorthEast(Dictionary<CellPositionXY, EdgeObjectCellDirection> cellPositionsDictionary, CellPositionXY buildableObjectOriginCellPosition, FourDirectionalRotation fourDirectionalDirection, int i)
        {
            switch (fourDirectionalDirection)
            {
                case FourDirectionalRotation.North:
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x, buildableObjectOriginCellPosition.y + 1 + i, EdgeObjectCellDirection.East);
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x + 1, buildableObjectOriginCellPosition.y + 1 + i, EdgeObjectCellDirection.West);
                    break;
                case FourDirectionalRotation.East:
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x + 1 + i, buildableObjectOriginCellPosition.y, EdgeObjectCellDirection.North);
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x + 1 + i, buildableObjectOriginCellPosition.y + 1, EdgeObjectCellDirection.South);
                    break;
                case FourDirectionalRotation.South:
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x, buildableObjectOriginCellPosition.y - i, EdgeObjectCellDirection.East);
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x + 1, buildableObjectOriginCellPosition.y - i, EdgeObjectCellDirection.West);
                    break;
                case FourDirectionalRotation.West:
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x - i, buildableObjectOriginCellPosition.y, EdgeObjectCellDirection.North);
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x - i, buildableObjectOriginCellPosition.y + 1, EdgeObjectCellDirection.South);
                    break;
            }
        }

        private void HandleEdgeObjectSouthEast(Dictionary<CellPositionXY, EdgeObjectCellDirection> cellPositionsDictionary, CellPositionXY buildableObjectOriginCellPosition, FourDirectionalRotation fourDirectionalDirection, int i)
        {
            switch (fourDirectionalDirection)
            {
                case FourDirectionalRotation.North:
                    ...
                    break;
                case FourDirectionalRotation.East:
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x + 1 + i, buildableObjectOriginCellPosition.y, EdgeObjectCellDirection.South);
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x + 1 + i, buildableObjectOriginCellPosition.y - 1, EdgeObjectCellDirection.North);
                    break;
                case FourDirectionalRotation.South:
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x, buildableObjectOriginCellPosition.y - 1 - i, EdgeObjectCellDirection.East);
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x + 1, buildableObjectOriginCellPosition.y - 1 - i, EdgeObjectCellDirection.West);
                    break;
                case FourDirectionalRotation.West:
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x - i, buildableObjectOriginCellPosition.y, EdgeObjectCellDirection.South);
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x - i, buildableObjectOriginCellPosition.y - 1, EdgeObjectCellDirection.North);
                    break;
            }
        }

        private void HandleEdgeObjectSouthWest(Dictionary<CellPositionXY, EdgeObjectCellDirection> cellPositionsDictionary, CellPositionXY buildableObjectOriginCellPosition, FourDirectionalRotation fourDirectionalDirection, int i)
        {
            switch (fourDirectionalDirection)
            {
                case FourDirectionalRotation.North:
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x, buildableObjectOriginCellPosition.y + i, EdgeObjectCellDirection.West);
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x - 1, buildableObjectOriginCellPosition.y + i, EdgeObjectCellDirection.East);
                    break;
                case FourDirectionalRotation.East:
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x + i, buildableObjectOriginCellPosition.y, EdgeObjectCellDirection.South);
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x + i, buildableObjectOriginCellPosition.y - 1, EdgeObjectCellDirection.North);
                    break;
                case FourDirectionalRotation.South:
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x, buildableObjectOriginCellPosition.y - 1 - i, EdgeObjectCellDirection.West);
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x - 1, buildableObjectOriginCellPosition.y - 1 - i, EdgeObjectCellDirection.East);
                    break;
                case FourDirectionalRotation.West:
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x - 1 - i, buildableObjectOriginCellPosition.y, EdgeObjectCellDirection.South);
                    AddEdgeObjectCellPosition(cellPositionsDictionary, buildableObjectOriginCellPosition.x - 1 - i, buildableObjectOriginCellPosition.y - 1, EdgeObjectCellDirection.North);
        ...
        private void HandleEdgeObjectNorthWest(Dictionary<CellPositionXY, EdgeObjectCellDirection> cellPositionsDictionary, CellPositionXY buildableObjectOriginCellPosition, FourDirectionalRotation fourDirectionalDirection, int i)
        {
            switch (fourDirectionalDirection)
            {
                case FourDirectionalRotation.North:
        ...
        private void AddEdgeObjectCellPosition(Dictionary<CellPositionXY, EdgeObjectCellDirection> cellPositionsDictionary, int x, int y, EdgeObjectCellDirection direction)
        {
            cellPositionsDictionary[new CellPositionXY(x, y)] = direction;
        }

        private bool TryPlaceBuildableEdgeObjectSinglePlacement(Dictionary<CellPositionXY, EdgeObjectCellDirection> cellPositionsDictionary, Vector3 modifiedCellWorldPosition, Vector3 offset,
            ...
            out BuildableEdgeObject buildableEdgeObject, BuildableEdgeObject originalBuildableEdgeObject = null)
        {
            ...
            foreach (KeyValuePair<CellPositionXY, EdgeObjectCellDirection> cellPosition in cellPositionsDictionary)
            {
                List<CellPositionXY> adjacentCellPositions =  new List<CellPositionXY>();

                if (gridList[activeVerticalGridIndex].IsWithinGridBounds(cellPosition.Key))
                {
                    isWithinGridBounds = true;
                    foreach (KeyValuePair<CellPositionXY, EdgeObjectCellDirection> edgeCellDirection in cellPositionsDictionary)
                    {
                        if (edgeCellDirection.Value == cellPosition.Value) adjacentCellPositions.Add(edgeCellDirection.Key);
                    }
                }

                foreach (CellPositionXY adjacentCellPosition in adjacentCellPositions)
                {
                ...
                foreach (KeyValuePair<CellPositionXY, EdgeObjectCellDirection> cellPosition in cellPositionsDictionary)
                {
                    ...
                    switch (cellPosition.Value)
                    {
                        case EdgeObjectCellDirection.North: 
                            if (gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableEdgeObjectNorthData().ContainsKey(buildableEdgeObjectSO.buildableEdgeObjectCategorySO))
                            {
                                BuildableObject buildableObject = gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableEdgeObjectNorthData()[buildableEdgeObjectSO.buildableEdgeObjectCategorySO];
                                if (!ignoreReplacement && buildableObject.GetBuildableObjectSO().isObjectReplacable && gridManager.TryGetBuildableObjectDestroyer(out BuildableObjectDestroyer buildableObjectDestroyer))
                                {
                                    isObjectReplacing = true;
                        ...
                        case EdgeObjectCellDirection.East: 
                            if (gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableEdgeObjectEastData().ContainsKey(buildableEdgeObjectSO.buildableEdgeObjectCategorySO))
                            {
                                BuildableObject buildableObject = gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableEdgeObjectEastData()[buildableEdgeObjectSO.buildableEdgeObjectCategorySO];
                                if (!ignoreReplacement && buildableObject.GetBuildableObjectSO().isObjectReplacable && gridManager.TryGetBuildableObjectDestroyer(out BuildableObjectDestroyer buildableObjectDestroyer))
                                {
                                    isObjectReplacing = true;
                                    buildableObjectDestroyer.SetInputDestroyBuildableObject(buildableObject, true, true);
                        ...
                        case EdgeObjectCellDirection.South: 
                            if (gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableEdgeObjectSouthData().ContainsKey(buildableEdgeObjectSO.buildableEdgeObjectCategorySO))
                            {
                                BuildableObject buildableObject = gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableEdgeObjectSouthData()[buildableEdgeObjectSO.buildableEdgeObjectCategorySO];
                                if (!ignoreReplacement && buildableObject.GetBuildableObjectSO().isObjectReplacable && gridManager.TryGetBuildableObjectDestroyer(out BuildableObjectDestroyer buildableObjectDestroyer))
                                {
                                    isObjectReplacing = true;
                                    buildableObjectDestroyer.SetInputDestroyBuildableObject(buildableObject, true, true);
                        ...
                        case EdgeObjectCellDirection.West: 
                            if (gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableEdgeObjectWestData().ContainsKey(buildableEdgeObjectSO.buildableEdgeObjectCategorySO))
                            {
                                BuildableObject buildableObject = gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableEdgeObjectWestData()[buildableEdgeObjectSO.buildableEdgeObjectCategorySO];
                                if (!ignoreReplacement && buildableObject.GetBuildableObjectSO().isObjectReplacable && gridManager.TryGetBuildableObjectDestroyer(out BuildableObjectDestroyer buildableObjectDestroyer))
                                {
                                    isObjectReplacing = true;
                                    buildableObjectDestroyer.SetInputDestroyBuildableObject(buildableObject, true, true);
            ...
            if (isWithinGridBounds && isBuildable)
            {
                foreach (KeyValuePair<CellPositionXY, EdgeObjectCellDirection> cellPosition in cellPositionsDictionary)
                {
                    ...
                    if (buildableEdgeObjectSO.affectByAreaEnablers && IsEdgeObjectEnabledByGridAreaEnablers(gridList[activeVerticalGridIndex], buildableEdgeObjectSO, new Vector2Int(cellPosition.Key.x, cellPosition.Key.y), cellPosition.Value))
                    {
                        isBuildable = true;
                        continue;
                    }
                    ...
                    if (buildableEdgeObjectSO.affectByAreaDisablers && IsEdgeObjectDisabledByGridAreaDisablers(gridList[activeVerticalGridIndex], buildableEdgeObjectSO, new Vector2Int(cellPosition.Key.x, cellPosition.Key.y), cellPosition.Value))
                    {
                        isBuildable = false;
                        break;
                    }
        ...
        private void InitializeBuildableEdgeObjectPlacement(Dictionary<CellPositionXY, EdgeObjectCellDirection> cellPositionsDictionary, Vector3 worldPosition, Vector3 offset, BuildableEdgeObjectSO buildableEdgeObjectSO, 
            ...
            bool ignoreBuildConditions, int activeVerticalGridIndex, bool byPassEventsAndMessages, out BuildableEdgeObject buildableEdgeObject, BuildableEdgeObject originalBuildableEdgeObject = null)
        {
            ...
            foreach (KeyValuePair<CellPositionXY, EdgeObjectCellDirection> cellPosition in cellPositionsDictionary)
            {
                cellPositionList.Add(new Vector2Int(cellPosition.Key.x, cellPosition.Key.y));
                newCellPositionsDictionary[new Vector2Int(cellPosition.Key.x, cellPosition.Key.y)] = cellPosition.Value;
            }
            ...
            {
                if (!gridList[activeVerticalGridIndex].IsWithinGridBounds(cellPosition.Key)) continue;
                GridCellData gridCellData = gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y);      // Get the GridCellData Copy
                ...
                gridList[activeVerticalGridIndex].SetCellData(cellPosition.Key.x, cellPosition.Key.y, gridCellData);                    // Write the modified GridCellData back to the original
                if (!byPassEventsAndMessages) RaiseActiveBuildableSOChangeEventsAndMessages(this, new Vector2Int(cellPosition.Key.x, cellPosition.Key.y));
            }
        }
            ...
            out BuildableCornerObject spawnnedBuildableCornerObject, BuildableObjectSO.RandomPrefabs buildableObjectSORandomPrefab = null, BuildableCornerObject originalBuildableCornerObject = null)
        {
            ...
            if (!isObjectMoving)
            {
                ICommand command = new CommandPlaceBuildableCornerObject(this, buildableObjectOriginCellPosition, closestCornerWorldPosition, default, 
                    buildableCornerObjectSO, fourDirectionalDirection, eightDirectionalDirection, freeRotation, cornerObjectOriginCellDirection, buildableObjectSORandomPrefab, ignoreCustomConditions,
            ...
            EightDirectionalRotation eightDirectionalDirection, float freeRotation, int activeVerticalGridIndex, bool byPassEventsAndMessages, bool invokedAsSecondaryPlacement = false)
        {
            ...
            {
                InitializeBuildableCornerObjectSinglePlacement(worldPosition, activeBuildableCornerObjectSO, fourDirectionalDirection, eightDirectionalDirection, freeRotation, ignoreBuildConditions, 
                    ignoreReplacement, activeVerticalGridIndex, byPassEventsAndMessages, invokedAsSecondaryPlacement);
            }
            else
            {
                ...
                {
                    case CornerObjectPlacementType.BoxPlacement:
                        InitializeBuildableCornerObjectBoxPlacement(activeBuildableCornerObjectSO, fourDirectionalDirection, eightDirectionalDirection, freeRotation, ignoreBuildConditions, ignoreReplacement, 
                            activeVerticalGridIndex, byPassEventsAndMessages, invokedAsSecondaryPlacement);
                    break;
                    case CornerObjectPlacementType.WireBoxPlacement:
                        InitializeBuildableCornerObjectWireBoxPlacement(activeBuildableCornerObjectSO, fourDirectionalDirection, eightDirectionalDirection, freeRotation, ignoreBuildConditions, ignoreReplacement, 
                            activeVerticalGridIndex, byPassEventsAndMessages, invokedAsSecondaryPlacement);
                    break;
                    case CornerObjectPlacementType.FourDirectionWirePlacement:
                        InitializeBuildableCornerObjectFourDirectionWirePlacement(activeBuildableCornerObjectSO, fourDirectionalDirection, eightDirectionalDirection, freeRotation, ignoreBuildConditions, 
                            ignoreReplacement, activeVerticalGridIndex, byPassEventsAndMessages, invokedAsSecondaryPlacement);
                    break;
                    case CornerObjectPlacementType.LShapedPlacement:
                        InitializeBuildableCornerObjectLShapedPlacement(activeBuildableCornerObjectSO, fourDirectionalDirection, eightDirectionalDirection, freeRotation, ignoreBuildConditions, ignoreReplacement, 
                            activeVerticalGridIndex, byPassEventsAndMessages, invokedAsSecondaryPlacement);
                    break;
                }
            }
            ...
            bool ignoreCustomConditions, bool ignoreReplacement, int verticalGridIndex, bool byPassEventsAndMessages, bool invokedAsSecondaryPlacement)
        {
            ...
            ICommand command = new CommandPlaceBuildableCornerObject(this, cellPosition, cellWorldPosition, offset, buildableCornerObjectSO, fourDirectionalDirection, eightDirectionalDirection, freeRotation, 
                cornerObjectOriginCellDirection, buildableObjectSORandomPrefab, ignoreCustomConditions, ignoreReplacement, verticalGridIndex, byPassEventsAndMessages, invokedAsSecondaryPlacement, null);
        ...
        {
            Dictionary<CellPositionXY, CornerObjectCellDirection> cellPositionsDictionary = GetCornerObjectCellPositionDictionary(new CellPositionXY(buildableObjectOriginCellPosition.x, buildableObjectOriginCellPosition.y), 
        ...
        public Vector3 GetClosestCorner(Vector3 cellWorldPosition, CornerObjectCellDirection cornerObjectOriginCellDirection)
        {
            ...
            switch (cornerObjectOriginCellDirection)
            {
                ...
                default: throw new ArgumentException("Invalid CornerObjectCellDirection provided");
            }
        }
        ...
        public Dictionary<CellPositionXY, CornerObjectCellDirection> GetCornerObjectCellPositionDictionary(CellPositionXY originCellPosition, CornerObjectCellDirection cornerObjectOriginCellDirection)
        {
            Dictionary<CellPositionXY, CornerObjectCellDirection> cellPositionsDictionary = new Dictionary<CellPositionXY, CornerObjectCellDirection>();
            ...
            switch (cornerObjectOriginCellDirection)
            {
                case CornerObjectCellDirection.NorthEast:
                    cellPositionsDictionary.Add(originCellPosition + new CellPositionXY(0, 1), CornerObjectCellDirection.SouthEast);
                    cellPositionsDictionary.Add(originCellPosition + new CellPositionXY(1, 1), CornerObjectCellDirection.SouthWest);
                    cellPositionsDictionary.Add(originCellPosition + new CellPositionXY(1, 0), CornerObjectCellDirection.NorthWest);
                    break;

                case CornerObjectCellDirection.SouthEast:
                    cellPositionsDictionary.Add(originCellPosition + new CellPositionXY(0, -1), CornerObjectCellDirection.NorthEast);
                    cellPositionsDictionary.Add(originCellPosition + new CellPositionXY(1, -1), CornerObjectCellDirection.NorthWest);
                    cellPositionsDictionary.Add(originCellPosition + new CellPositionXY(1, 0), CornerObjectCellDirection.SouthWest);
                    break;

                case CornerObjectCellDirection.SouthWest:
                    cellPositionsDictionary.Add(originCellPosition + new CellPositionXY(0, -1), CornerObjectCellDirection.NorthWest);
                    cellPositionsDictionary.Add(originCellPosition + new CellPositionXY(-1, -1), CornerObjectCellDirection.NorthEast);
                    cellPositionsDictionary.Add(originCellPosition + new CellPositionXY(-1, 0), CornerObjectCellDirection.SouthEast);
                    break;

                case CornerObjectCellDirection.NorthWest:
                    cellPositionsDictionary.Add(originCellPosition + new CellPositionXY(0, 1), CornerObjectCellDirection.SouthWest);
                    cellPositionsDictionary.Add(originCellPosition + new CellPositionXY(-1, 1), CornerObjectCellDirection.SouthEast);
                    cellPositionsDictionary.Add(originCellPosition + new CellPositionXY(-1, 0), CornerObjectCellDirection.NorthEast);
                    break;
            }
        ...
        private bool TryPlaceBuildableCornerObjectSinglePlacement(Dictionary<CellPositionXY, CornerObjectCellDirection> cellPositionsDictionary, Vector3 modifiedCellWorldPosition, Vector3 offset,
            ...
            bool invokedAsSecondaryPlacement, out BuildableCornerObject buildableCornerObject, BuildableCornerObject originalBuildableCornerObject = null)
        {
            ...
            if (isWithinGridBounds)
            {
                foreach (KeyValuePair<CellPositionXY, CornerObjectCellDirection> cellPosition in cellPositionsDictionary)
                {
                    ...
                    {
                        case CornerObjectCellDirection.NorthEast: 
                            if (gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableCornerObjectNorthEastData().ContainsKey(buildableCornerObjectSO.buildableCornerObjectCategorySO))
                            {
                                BuildableObject buildableObject = gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableCornerObjectNorthEastData()[buildableCornerObjectSO.buildableCornerObjectCategorySO];
                                if (!ignoreReplacement && buildableObject.GetBuildableObjectSO().isObjectReplacable && gridManager.TryGetBuildableObjectDestroyer(out BuildableObjectDestroyer buildableObjectDestroyer))
                                {
                                    isObjectReplacing = true;
                                    buildableObjectDestroyer.SetInputDestroyBuildableObject(buildableObject, true, true);
                        ...
                        break;
                        case CornerObjectCellDirection.SouthEast: 
                            if (gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableCornerObjectSouthEastData().ContainsKey(buildableCornerObjectSO.buildableCornerObjectCategorySO))
                            {
                                BuildableObject buildableObject = gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableCornerObjectSouthEastData()[buildableCornerObjectSO.buildableCornerObjectCategorySO];
                                if (!ignoreReplacement && buildableObject.GetBuildableObjectSO().isObjectReplacable && gridManager.TryGetBuildableObjectDestroyer(out BuildableObjectDestroyer buildableObjectDestroyer))
                                {
                                    isObjectReplacing = true;
                                    buildableObjectDestroyer.SetInputDestroyBuildableObject(buildableObject, true, true);
                        ...
                        break;
                        case CornerObjectCellDirection.SouthWest: 
                            if (gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableCornerObjectSouthWestData().ContainsKey(buildableCornerObjectSO.buildableCornerObjectCategorySO))
                            {
                                BuildableObject buildableObject = gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableCornerObjectSouthWestData()[buildableCornerObjectSO.buildableCornerObjectCategorySO];
                                if (!ignoreReplacement && buildableObject.GetBuildableObjectSO().isObjectReplacable && gridManager.TryGetBuildableObjectDestroyer(out BuildableObjectDestroyer buildableObjectDestroyer))
                                {
                                    isObjectReplacing = true;
                                    buildableObjectDestroyer.SetInputDestroyBuildableObject(buildableObject, true, true);
                        ...
                        break;
                        case CornerObjectCellDirection.NorthWest: 
                            if (gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableCornerObjectNorthWestData().ContainsKey(buildableCornerObjectSO.buildableCornerObjectCategorySO))
                            {
                                BuildableObject buildableObject = gridList[activeVerticalGridIndex].GetCellData(cellPosition.Key.x, cellPosition.Key.y).GetBuildableCornerObjectNorthWestData()[buildableCornerObjectSO.buildableCornerObjectCategorySO];
                                if (!ignoreReplacement && buildableObject.GetBuildableObjectSO().isObjectReplacable && gridManager.TryGetBuildableObjectDestroyer(out BuildableObjectDestroyer buildableObjectDestroyer))
                                {
                                    isObjectReplacing = true;
                                    buildableObjectDestroyer.SetInputDestroyBuildableObject(buildableObject, true, true);
            ...
            if (isWithinGridBounds && isBuildable)
            {
                foreach (KeyValuePair<CellPositionXY, CornerObjectCellDirection> cellPosition in cellPositionsDictionary)
                {
                    ...
                    if (buildableCornerObjectSO.affectByAreaEnablers && IsCornerObjectEnabledByGridAreaEnablers(gridList[activeVerticalGridIndex], buildableCornerObjectSO, new Vector2Int(cellPosition.Key.x, cellPosition.Key.y), cellPosition.Value))
                    {
                        isBuildable = true;
                        continue;
                    }
                    ...
                    if (buildableCornerObjectSO.affectByAreaDisablers && IsCornerObjectDisabledByGridAreaDisablers(gridList[activeVerticalGridIndex], buildableCornerObjectSO, new Vector2Int(cellPosition.Key.x, cellPosition.Key.y), cellPosition.Value))
                    {
                        isBuildable = false;
                        break;
                    }
        ...
        private void InitializeBuildableCornerObjectPlacement(Dictionary<CellPositionXY, CornerObjectCellDirection> cellPositionsDictionary, Vector3 modifiedCellWorldPosition, Vector3 offset, BuildableCornerObjectSO buildableCornerObjectSO, 
        ...
        public override void InvokeTryPlaceBuildableFreeObjectSinglePlacement(BuildableFreeObjectSO buildableFreeObjectSO, Vector3 worldPosition, FourDirectionalRotation fourDirectionalDirection, 
            ...
            BuildableFreeObject originalBuildableFreeObject = null)
        {
            CellPositionXY cellPosition = GetActiveGridCellPositionXY(worldPosition);
        ...
        private bool TryPlaceBuildableFreeObjectSinglePlacement(BuildableFreeObjectSO buildableFreeObjectSO, CellPositionXY cellPosition, Vector3 worldPosition, 
            ...
            BuildableFreeObject originalBuildableFreeObject = null)
        {
            ...
            void CheckConditions()
            {
                ...
                if (buildableFreeObjectSO.affectByAreaEnablers && IsFreeObjectEnabledByGridAreaEnablers(gridList[activeVerticalGridIndex], buildableFreeObjectSO, new Vector2Int(cellPosition.x, cellPosition.y)))
                {
                    isBuildable = true;
                    return;
                }
                ...
                if (buildableFreeObjectSO.affectByAreaDisablers && IsFreeObjectDisabledByGridAreaDisablers(gridList[activeVerticalGridIndex], buildableFreeObjectSO, new Vector2Int(cellPosition.x, cellPosition.y)))
                {
                    isBuildable = false;
                    return;
                }
        ...
        private void InitializeBuildableFreeObjectPlacement(BuildableFreeObjectSO buildableFreeObjectSO, CellPositionXY cellPosition, Vector3 worldPosition, 
            FourDirectionalRotation fourDirectionalDirection, EightDirectionalRotation eightDirectionalDirection, float freeRotation, Vector3 hitNormals, bool ignoreBuildConditions,
            int activeVerticalGridIndex, bool byPassEventsAndMessages, out BuildableFreeObject buildableFreeObject, BuildableObjectSO.RandomPrefabs buildableObjectSORandomPrefab = null,
            BuildableFreeObject originalBuildableFreeObject = null)
        {
            ...
            buildableFreeObject.SetupBuildableFreeObject(this, activeVerticalGridIndex, cellSize, buildableFreeObjectSO, activeBuildableObjectSORandomPrefab, worldPosition, 
                new Vector2Int(cellPosition.x, cellPosition.y), fourDirectionalDirection, eightDirectionalDirection, freeRotation, hitNormals);
            ...
            if (!byPassEventsAndMessages) RaiseActiveBuildableSOChangeEventsAndMessages(this, new Vector2Int(cellPosition.x, cellPosition.y));
        }
        ...
        public bool IsWithinActiveGridBounds(CellPositionXY cellPositionXY)
        {
            return activeGrid.IsWithinGridBounds(cellPositionXY);
        }

        public bool IsWithinGridBounds(CellPositionXY cellPositionXY, int verticalGridIndex)
        {
            return gridList[verticalGridIndex].IsWithinGridBounds(cellPositionXY);
        }
        ...
        private void RaiseGridSystemCreationEventsAndMessages(EasyGridBuilderPro easyGridBuilderPro, int verticalGridCount)
        {
            ...
            {
                Debug.Log($"EasyGridBuilderPro XY: {easyGridBuilderPro.name}: <color=green><b>Grid System Created!</b></color> Vertical Grid Count: {verticalGridCount}");
            }
        ...
        private void RaiseActiveGridModeChangeEventsAndMessages(EasyGridBuilderPro easyGridBuilderPro, GridMode gridMode)
        {
            ...
            {
                Debug.Log($"EasyGridBuilderPro XY: {this.name}: <color=green><b>Active Grid Mode Changed!</b></color> {gridMode}");
            }
        ...
        private void RaiseActiveBuildableSOChangeEventsAndMessages(EasyGridBuilderPro easyGridBuilderPro, BuildableObjectSO buildableObjectSO)
        {
            ...
            {
                Debug.Log($"EasyGridBuilderPro XY: {this.name}: <color=green><b>Active Buildable Object SO Changed!</b></color> {buildableObjectSO.name}");
            }
        ...
        private void RaiseActiveVerticalGridChangeEventsAndMessages(EasyGridBuilderPro easyGridBuilderPro, int activeVerticalGridIndex)
        {
            ...
            {
                Debug.Log($"EasyGridBuilderPro XY: {this.name}: <color=green><b>Active Vertical Grid Changed!</b></color> Vertical Grid Index: {activeVerticalGridIndex}");
            }
        ...
        private void RaiseActiveBuildableSOChangeEventsAndMessages(EasyGridBuilderPro easyGridBuilderPro, Vector2Int cellPosition)
        {
            ...
            {
                Debug.Log($"EasyGridBuilderPro XY: {easyGridBuilderPro.name}: <color=green><b>Cell Object Value Changed!</b></color> Cell Position: {cellPosition}");
            }
        ...
        private void RaiseBuildableObjectPlacementEventsAndMessages(EasyGridBuilderPro easyGridBuilderPro, BuildableObject buildableObject, bool byPassEventsAndMessages = false)
        {
                ...
                buildableEdgeObjectSO.placementType != EdgeObjectPlacementType.PaintPlacement)
            {
                ...
                {
                    Debug.Log($"EasyGridBuilderPro XY: {this.name}: Placement Type: {buildableEdgeObjectSO.placementType}: " +
            ...
            {
                Debug.Log($"EasyGridBuilderPro XY: {easyGridBuilderPro.name}: <color=green><b>Buildable Object Placed!</b></color> {buildableObject.name}");
            }
        ...
        private void RaiseOnBuildableObjectSOAddedEventsAndMessages(EasyGridBuilderPro easyGridBuilderPro, BuildableObjectSO buildableObjectSO)
        {
            ...
            {
                Debug.Log($"EasyGridBuilderPro XY: {easyGridBuilderPro.name}: <color=green><b>New Buildable Object SO Added! </b></color> {buildableObjectSO}");
            }
        ...
        private void RaiseOnBuildableObjectSORemovedEventsAndMessages(EasyGridBuilderPro easyGridBuilderPro, BuildableObjectSO buildableObjectSO)
        {
            ...
            {
                Debug.Log($"EasyGridBuilderPro XY: {easyGridBuilderPro.name}: <color=green><b>New Buildable Object SO Removed! </b></color> {buildableObjectSO}");
            }
        ...
        {
            return gridList.IndexOf(grid as GridXY);
        }
        ...
        public Vector3 GetActiveGridCellWorldPosition(CellPositionXY cellPositionXY)
        {
            return activeGrid.GetCellWorldPosition(cellPositionXY);
        }
        ...
        public Vector3 GetCellWorldPosition(CellPositionXY cellPositionXY, int verticalGridIndex)
        {
            return gridList[verticalGridIndex].GetCellWorldPosition(cellPositionXY);
        }

        public override Vector2Int GetActiveGridCellPosition(Vector3 worldPosition)
        {
            return new Vector2Int(activeGrid.GetCellPosition(worldPosition).x, activeGrid.GetCellPosition(worldPosition).y);
        }

        public override Vector2Int GetCellPosition(Vector3 worldPosition, int verticalGridIndex)
        {
            return new Vector2Int(gridList[verticalGridIndex].GetCellPosition(worldPosition).x, gridList[verticalGridIndex].GetCellPosition(worldPosition).y);
        }
        ...
        public CellPositionXY GetActiveGridCellPositionXY(Vector3 worldPosition)
        {
            return activeGrid.GetCellPosition(worldPosition);
        }

        public CellPositionXY GetCellPositionXY(Vector3 worldPosition, int verticalGridIndex)
        {
        ...
        {
            foreach (GridXY grid in gridList)
            {
                grid.UpdateGeneratedTexture();
            }
        }
        
        public override void SetRuntimeObjectGridGeneratedTextureCellToDefault(Vector2Int cellPosition, bool affectAllVerticalGrids, Grid grid)
        {
            if (affectAllVerticalGrids)
            {
                foreach (GridXY gridSystem in gridList)
                {
        ...
        public override void SetRuntimeObjectGridGeneratedTextureCellColor(Vector2Int cellPosition, Color cellColor, bool affectAllVerticalGrids, Grid grid)
        {
            if (affectAllVerticalGrids)
            {
                foreach (GridXY gridSystem in gridList)
