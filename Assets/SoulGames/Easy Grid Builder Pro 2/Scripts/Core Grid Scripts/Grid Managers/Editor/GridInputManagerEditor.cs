...
{
    [CustomEditor(typeof(GridInputManager))]
    public class GridInputManagerEditor : Editor
    {
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            if (visualTreeAsset == null)
            {
                ...
                string uxmlPath = System.IO.Path.Combine(scriptDirectory, "GridInputManagerEditor.uxml");
                visualTreeAsset = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(uxmlPath);
            }
            visualTreeAsset.CloneTree(root);

            MonoScript script = MonoScript.FromMonoBehaviour((GridInputManager)target);
